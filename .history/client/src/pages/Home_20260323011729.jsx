    ...
    <div className="price-slider-wrap">
      ...
      <div className="price-inputs">
        <div className="price-input-box">
          ...
          <input type="number" value={minVal} min={min} step={100} placeholder="0"
            onChange={(e) => onChange([Number(e.target.value) || 0, maxRaw])} />
        </div>
...
const Home = () => {
  ...
  const [priceStats, setPriceStats] = useState({ min: 0, max: 100000, buckets: [] })
  ...
  const fetchPriceStats = async () => {
    try {
      ...
      const { min_price, max_price, buckets } = response.data
      if (min_price != null && max_price != null) {
        const effectiveMax = max_price <= min_price ? min_price + 10000 : max_price
        setPriceStats({ min: min_price, max: effectiveMax, buckets })
        setPriceRange([min_price, ''])
      }
    } catch (error) {
